(() => {
'use strict';
// NEXA V52.12 — MULTI-SERVER AUTH CLEAN SOURCE — 2026-08-25
...
function decorativeName(value){
  for(const ch of String(value||'')){
    ...
    if((cp>=0x1D400&&cp<=0x1D7FF)||(cp>=0xFF00&&cp<=0xFFEF)||(cp>=0x2460&&cp<=0x24FF))return true;
  }
  return false;
}
...
function styles(){
  ...
  .nexa-auth-logo{width:58px;height:58px;border-radius:19px;margin:0 auto 11px;display:grid;place-items:center;overflow:hidden;background:linear-gradient(135deg,#8241ff,#158fff);box-shadow:0 0 32px rgba(93,80,255,.42)}
  ...
  .nexa-auth-tab{border:0;border-radius:11px;padding:11px 8px;color:#8f98bb;background:transparent;font-weight:900;letter-spacing:.06em}
  ...
  .nexa-auth-submit{width:100%;border:0;border-radius:13px;padding:13px 15px;font-weight:950;letter-spacing:.05em;color:#fff;background:linear-gradient(135deg,#7a3eff,#168fff)}
  ...
  .nexa-owner-lock{position:absolute;right:12px;bottom:11px;width:34px;height:34px;border:0;border-radius:50%;background:transparent;color:rgba(221,222,255,.55);font-size:17px}
...
function stars(container){
  for(let i=0;i<55;i++){const s=document.createElement('span');s.className='nexa-star';s.style.left=(Math.random()*100)+'%';s.style.top=(Math.random()*100)+'%';s.style.setProperty('--d',(2.2+Math.random()*4.8)+'s');s.style.setProperty('--delay',(-Math.random()*5)+'s');container.appendChild(s)}
}
function markup(){
  document.getElementById('nexa-auth-gate')?.remove();
  gate=document.createElement('div');gate.id='nexa-auth-gate';
  gate.innerHTML=`
  ...
  <section class="nexa-auth-card">
    ...
    <div id="nexa-pane-login" class="nexa-auth-pane active">
      <h2>Welcome Back</h2>
      <p class="sub">Use your Game ID, State and NEXA Password to continue.</p>
      <form id="nexa-login-form" class="nexa-auth-form">
        <label>Game ID<input id="nexa-login-game-id" required autocomplete="username" inputmode="numeric" placeholder="Enter your Game ID"></label>
        ...
        <label>NEXA Password<input id="nexa-login-password" required minlength="8" type="password" autocomplete="current-password" placeholder="Enter your password"></label>
        <button class="nexa-auth-submit" type="submit">LOGIN</button>
      </form>
      <div class="nexa-auth-help"><span>Any linked Game ID can later access the same NEXA profile.</span><button id="nexa-forgot" class="nexa-auth-link" type="button">Forgot Password?</button></div>
    </div>
    <div id="nexa-pane-create" class="nexa-auth-pane">
      ...
      <form id="nexa-create-form" class="nexa-auth-form">
        <label>Main Game ID<input id="nexa-create-game-id" required autocomplete="username" inputmode="numeric" placeholder="Enter your Main Game ID"><span class="nexa-field-hint">Your first account becomes your Main Account.</span></label>
        ...
        <label>Alliance<select id="nexa-create-alliance" required><option value="">Loading alliances…</option></select></label>
        <label id="nexa-custom-alliance-wrap" style="display:none">Alliance Tag<input id="nexa-create-custom-alliance" maxlength="12" placeholder="Enter alliance tag"></label>
...
async function loadAlliances(){const select=$('nexa-create-alliance');try{const response=await fetch(`${SUPABASE_URL}/rest/v1/rpc/get_public_nexa_alliances`,{method:'POST',headers:{apikey:SUPABASE_KEY,Authorization:`Bearer ${SUPABASE_KEY}`,'Content-Type':'application/json'},body:'{}',cache:'no-store'});const raw=await response.text();let rows=[];try{rows=raw?JSON.parse(raw):[]}catch{}if(!response.ok)throw new Error(rows?.message||rows?.error||raw||'Could not load alliances');if(!Array.isArray(rows))rows=[];select.innerHTML='<option value="">Select alliance</option>'+rows.map(a=>`<option value="${esc(a.id)}">${esc(a.tag)}</option>`).join('')+'<option value="not-listed">Not Listed</option>'}catch(e){select.innerHTML='<option value="">Could not load alliances</option><option value="not-listed">Not Listed</option>';msg(e.message,'error')}}
...
async function login(e){e.preventDefault();msg('Signing in…');try{const stateNumber=Number(String($('nexa-login-state').value||'').replace(/\D/g,''));if(!stateNumber)throw new Error('Enter your state.');const data=await api('/api/nexa-auth-login',{game_id:$('nexa-login-game-id').value.trim(),state_number:stateNumber,password:$('nexa-login-password').value});const {error}=await sb.auth.setSession({access_token:data.session.access_token,refresh_token:data.session.refresh_token});if(error)throw error;msg('Welcome back.','good');await reconcile();location.reload()}catch(e){msg(e.message,'error')}}
async function create(e){e.preventDefault();msg('');const gameId=$('nexa-create-game-id').value.trim(),ign=$('nexa-create-name').value.trim(),p1=$('nexa-create-password').value,p2=$('nexa-create-password2').value,alliance=$('nexa-create-alliance').value,stateNumber=Number(String($('nexa-create-state').value||'').replace(/\D/g,''));if(!stateNumber)return msg('Enter your state.','error');if(decorativeName(ign))return msg('Decorative characters are not supported. Please use standard letters for your In-Game Name.','error');if(p1!==p2)return msg('Passwords do not match.','error');if(p1.length<8)return msg('NEXA Password must be at least 8 characters.','error');if(!alliance)return msg('Select your alliance.','error');try{const data=await api('/api/nexa-auth-create',{game_id:gameId,in_game_name:ign,state_number:stateNumber,alliance_id:alliance==='not-listed'?null:Number(alliance),custom_alliance_tag:alliance==='not-listed'?$('nexa-create-custom-alliance').value.trim():null,password:p1});const {error}=await sb.auth.setSession({access_token:data.session.access_token,refresh_token:data.session.refresh_token});if(error)throw error;await reconcile();location.reload()}catch(e){msg(e.message,'error')}}
async function init(){styles();markup();sb=window.supabase.createClient(SUPABASE_URL,SUPABASE_KEY);document.querySelectorAll('[data-auth-tab]').forEach(btn=>btn.addEventListener('click',()=>show(btn.dataset.authTab)));$('nexa-login-form').addEventListener('submit',login);$('nexa-create-form').addEventListener('submit',create);$('nexa-owner-lock').addEventListener('click',()=>{location.href='/owner-access.html'});$('nexa-forgot').addEventListener('click',()=>msg('Forgot your password? Ask your R5 or an authorized NEXA administrator for help.',''));$('nexa-create-alliance').addEventListener('change',e=>{const custom=e.target.value==='not-listed';$('nexa-custom-alliance-wrap').style.display=custom?'grid':'none';$('nexa-create-custom-alliance').required=custom});window.NEXAAuth={show,reconcile};await loadAlliances();await reconcile();sb.auth.onAuthStateChange(()=>setTimeout(reconcile,0))}
