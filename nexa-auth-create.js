import {
  SUPABASE_URL,SUPABASE_ANON,reply,serviceKey,normalizeGameId,cleanName,rest
...
export default async function handler(req,res){
  ...
  try{
    ...
    if(!allianceId&&!customAlliance)return reply(res,400,{error:'Select an alliance or choose Not Listed.'});
    ...
    const existingPlayer=await rest(`player_accounts?player_id=eq.${encodeURIComponent(rawGameId)}&select=id&limit=1`);
    if(existingPlayer?.length)return reply(res,409,{error:'That Game ID is already registered in NEXA.'});

    const email=`${normalized.replace(/[^a-z0-9_-]/g,'')}@accounts.nexa.invalid`;
    const user=await adminCreate(email,password,{
      in_game_name:ign,nexa_game_id:rawGameId,auth_method:'game_id'
    });
    createdId=user.id;
