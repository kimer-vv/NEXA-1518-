/* NEXA Profile Owner V31 — FINAL CLEAN HORIZONTAL PROFILE
   Reclaims Profile from V29/legacy owners and keeps a single compact horizontal workspace.
   Single visual owner. Native iOS rails. No MutationObserver. No manual scrollLeft.
*/
(()=>{
'use strict';
if(window.__NEXA_PROFILE_V31_INITIALIZED__) return;
window.__NEXA_PROFILE_V31_INITIALIZED__=true;
/* V31 deliberately takes ownership even if V29 already ran. */
...
document.addEventListener('click',e=>{
 const planet=e.target.closest?.('[data-nexa-profile],[data-account-constellation-id]');if(planet){setTimeout(()=>load(planet.dataset.nexaProfile||planet.dataset.accountConstellationId),140);setTimeout(()=>{mount();paint()},420);return}
 if(e.target.closest?.('#nexa-profile-launcher,#nexa-profile-launcher-section')){setTimeout(()=>load(),150);setTimeout(()=>{mount();paint()},450);return}
