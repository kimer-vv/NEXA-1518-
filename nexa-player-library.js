/* NEXA Player Library Profile — v1 */
(()=>{
  'use strict';
  let accountId=null;
  ...
  function addStyles(){
    ...
    s.textContent=`
      ...
      .nexa-lib-head{display:grid;grid-template-columns:52px 1fr auto;gap:10px;align-items:center}.nexa-lib-avatar{width:52px;height:52px;border-radius:50%;object-fit:cover;border:2px solid #8062ff;background:#101b3b}.nexa-lib-fallback{display:grid;place-items:center;color:#ccbfff;font-weight:950;font-size:18px}.nexa-lib-head h4{margin:0;color:#fff}.nexa-lib-head small{display:block;color:#8290b8;margin-top:3px}.nexa-lib-owned{display:flex;align-items:center;gap:5px;color:#81ddff;font-size:11px;font-weight:900}.nexa-lib-owned input{width:20px;height:20px}
  ...
  async function render(key){
    ...
    c.innerHTML=`<div class="nexa-lib-grid">${items.map(item=>{
      ...
      const img=item.image_url?`<img class="nexa-lib-avatar" src="${esc(item.image_url)}" alt="">`:`<div class="nexa-lib-avatar nexa-lib-fallback">${esc(initials)}</div>`;
  ...
  document.addEventListener('click',e=>{
    const planet=e.target.closest('[data-nexa-profile]'); if(planet){accountId=planet.dataset.nexaProfile; setTimeout(()=>{installTabs();render('heroes');},120);}
  ...
  new MutationObserver(()=>{if(document.querySelector('#nexa-profile-modal.open'))installTabs();}).observe(document.documentElement,{subtree:true,attributes:true,attributeFilter:['class']});
  addStyles(); installTabs();
})();
