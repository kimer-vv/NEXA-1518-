...
export default async function handler(req,res){
  ...
  try{
    ...
    // Owner may continue using the private Discord route.
    const serviceRoleRows=await rest(
      `user_roles?user_id=eq.${encodeURIComponent(user.id)}&select=role&limit=1`
    );
    const role=String(serviceRoleRows?.[0]?.role||'player').toLowerCase();
    const provider=String(user?.app_metadata?.provider||'');

    if(provider==='discord'){
      return reply(res,200,{allowed:role==='owner',role,method:'discord'});
    }
    ...
    return reply(res,e.status||500,{error:e.message||'Could not verify NEXA session.'});
  }
}
