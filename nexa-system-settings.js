const {SUPABASE_URL,json,userRole,bypassCookie}=require('./_nexa-maintenance-common');
...
module.exports=async function(req,res){
 try{
   ...
   if(req.method==='POST'){
     ...
     if(enabled)bypassCookie(res); // establish Owner escape route BEFORE turning site off
     await setSetting(service,enabled);
     return json(res,200,{maintenance_mode:enabled});
   }
   return json(res,405,{error:'Method not allowed.'});
 }catch(e){return json(res,e.status||500,{error:e.message||'System Operations failed.'});}
};