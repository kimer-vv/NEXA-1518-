...
function localStartText(date){if(!date)return'<b>Not scheduled.</b> No automatic Transfer phase announcements will run until a Game/Server start date is saved.';const d=new Date(`${date}T00:00:00Z`);if(Number.isNaN(d.getTime()))return'Invalid date.';return `<b>Game/Server:</b> ${esc(fmtDateUtc(d))} · 00:00 UTC<br><b>Your local time:</b> ${esc(fmtLocal(d))}`}
...
function readiness(){const checks=[['Discord server',!!cfg?.guild_id],['New Applications channel',!!cfg?.applications_channel_id],['Applicant Operations channel',!!cfg?.applicant_operations_channel_id],['Transfer Announcements channel',!!cfg?.reminders_channel_id],['Invite Operations channel',!!cfg?.invites_channel_id],['Integration enabled',cfg?.enabled===true]];const good=checks.every(x=>x[1]);return `<b>${good?'Bot Ready ✓':'Setup incomplete'}</b><br>${checks.map(([n,ok])=>`${ok?'✓':'○'} ${esc(n)}`).join(' · ')}`}
function compactSummary(){const count=[cfg?.applications_channel_id,cfg?.applicant_operations_channel_id,cfg?.reminders_channel_id,cfg?.invites_channel_id].filter(Boolean).length,start=dateValue();return `${cfg?.guild_id&&cfg?.enabled?'Connected':'Not Connected'} · ${count}/4 channels configured · Reminders ${cfg?.reminders_enabled!==false?'On':'Off'} · ${start?`Start ${esc(start)} · 00:00 UTC`:'Start Not scheduled'}`}
...
<li><b>New Applications Channel:</b> receives automatic alerts when a new Transfer application is submitted.</li>
<li><b>Applicant Operations Channel:</b> the dedicated channel for the <b>/applicant</b> and <b>/applicants</b> command workflow.</li>
...
<li><b>Test Notifications:</b> sends a clearly marked TEST message to each configured notification route without creating applicants, changing invites, or changing the Transfer timeline.</li>
...
async function testNotification(kind,label){
  ...
  const ids=['ndTestApplication','ndTestApplicantOps','ndTestAnnouncement','ndTestWarning','ndTestInvite'];ids.forEach(id=>{const b=byId(id);if(b)b.disabled=true});
  try{
