...
function compactSummary(){const count=[cfg?.applications_channel_id,cfg?.reminders_channel_id,cfg?.invites_channel_id].filter(Boolean).length,start=dateValue();return `${cfg?.guild_id&&cfg?.enabled?'Connected':'Not Connected'} · ${count}/3 channels configured · Reminders ${cfg?.reminders_enabled!==false?'On':'Off'} · ${start?`Start ${esc(start)} · 00:00 UTC`:'Start Not scheduled'}`}
