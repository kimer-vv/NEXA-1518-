...
(()=>{
  ...
  function addStyles(){
    ...
    s.textContent=`
      ...
      .nexa-lib-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(84px,1fr));gap:18px 10px}.nexa-lib-filterbar{display:flex;gap:8px;overflow-x:auto;margin-bottom:16px}.nexa-lib-filter{flex:0 0 auto;border:1px solid rgba(137,110,255,.35);border-radius:999px;padding:8px 13px;background:#0a1230;color:#9da9c9;font-weight:900}.nexa-lib-filter.active{color:#fff;background:linear-gradient(135deg,#632dcc,#126a98)}.nexa-lib-card{padding:0!important;border:0!important;background:transparent!important;box-shadow:none!important;text-align:center}.nexa-lib-head{display:flex!important;flex-direction:column;gap:7px!important}.nexa-lib-avatar-wrap{width:76px!important;height:76px!important;border-radius:50%!important;margin:auto}.nexa-lib-head h4{font-size:11px}.nexa-lib-head small{font-size:8px}.nexa-lib-owned,.nexa-lib-fields,.nexa-lib-save,.nexa-lib-status{display:none!important}.nexa-lib-card{border:1px solid rgba(137,110,255,.25);border-radius:18px;padding:13px;background:linear-gradient(145deg,rgba(19,25,54,.94),rgba(6,12,29,.96));box-shadow:0 12px 34px rgba(0,0,0,.2)}
      ...
      .nexa-lib-card.open{position:fixed!important;z-index:2147483000;inset:auto 14px 5dvh;max-width:520px;max-height:88dvh;overflow:auto;margin:auto;padding:18px!important;border:1px solid #754de0!important;border-radius:24px!important;background:linear-gradient(155deg,#14183c,#050918 68%)!important;box-shadow:0 25px 90px #000!important;text-align:left}.nexa-lib-card.open .nexa-lib-head{display:grid!important;grid-template-columns:76px 1fr auto!important}.nexa-lib-card.open .nexa-lib-owned{display:flex!important}.nexa-lib-card.open .nexa-lib-fields{display:grid!important}.nexa-lib-card.open .nexa-lib-save{display:block!important}.nexa-lib-card.open .nexa-lib-status{display:block!important}.nexa-lib-card.open .nexa-lib-avatar-wrap{margin:0}.nexa-lib-card.open:before{content:'PROFILE CONFIGURATION';display:block;color:#77dcff;font-size:9px;font-weight:950;letter-spacing:.16em;margin-bottom:13px}.nexa-lib-card.open:after{content:'×';position:absolute;right:13px;top:8px;width:34px;height:34px;border-radius:50%;display:grid;place-items:center;background:#11182e;border:1px solid #3c4566;color:#fff;font-size:22px}.nexa-lib-card.open .nexa-lib-head{padding-right:34px}
      @media(max-width:520px){.nexa-lib-grid{grid-template-columns:repeat(3,minmax(0,1fr))}.nexa-profile-content{padding:12px!important}}
  ...
  function fields(item,row){
    ...
    if(item.item_type==='expert'){
      ...
      return `<label>AFFINITY / RELATIONSHIP<input data-f="level" type="number" inputmode="numeric" min="0" max="100" value="${esc(p.level||'')}"></label>${skills.map((s,i)=>`<label>${esc(s.name)}<input data-f="skill_${i}" type="number" inputmode="numeric" min="0" max="${Number(s.max_level)||20}" value="${esc((p.skills||[])[i]??'')}"><small style="display:block;margin-top:4px;line-height:1.3;color:#7382ab">${esc(s.effect||'')}</small></label>`).join('')}`;
    }
    if(item.item_type==='pet') return `<label>PET LEVEL<input data-f="level" type="number" inputmode="numeric" min="0" value="${esc(p.level||'')}"></label><label>SKILL LEVEL<input data-f="skill_level" type="number" inputmode="numeric" min="0" value="${esc(p.skill_level||'')}"></label>`;
  ...
  async function render(key){
    ...
    c.innerHTML=`${filterHtml}<div class="nexa-lib-grid">${visible.map(item=>{
  ...
  document.addEventListener('click',e=>{
    ...
    if(card&&!e.target.closest('input,select,.nexa-lib-save')){document.querySelectorAll('.nexa-lib-card.open').forEach(x=>{if(x!==card)x.classList.remove('open')});card.classList.toggle('open');return;}
