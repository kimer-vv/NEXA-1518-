/* NEXA Player Intelligence Galactic Library — v6.0 */
(()=>{
  'use strict';
  ...
  function addStyles(){
    ...
    s.textContent=`
      ...
      .nexa-lib-fields{grid-template-columns:1fr!important;gap:13px!important}.nexa-control{border:1px solid rgba(111,139,220,.2);border-radius:16px;padding:12px;background:rgba(4,10,27,.72)}.nexa-control-title{display:flex;justify-content:space-between;align-items:center;color:#8e9bc1;font-size:9px;font-weight:950;letter-spacing:.13em;margin-bottom:10px}.nexa-control-title b{color:#79dcff;font-size:12px}.nexa-choice-row{display:flex;gap:6px;flex-wrap:wrap}.nexa-choice{min-width:34px;height:34px;padding:0 10px;border:1px solid rgba(119,136,205,.25);border-radius:10px;background:#08132e;color:#9ba8cc;font-weight:900}.nexa-choice.active{color:white;border-color:var(--planet);background:color-mix(in srgb,var(--planet) 28%,#08132e);box-shadow:0 0 14px color-mix(in srgb,var(--planet) 25%,transparent)}.nexa-stars{display:flex;justify-content:center;gap:10px}.nexa-star{border:0;background:transparent;color:#44506f;font-size:31px;padding:0;filter:drop-shadow(0 0 0 transparent)}.nexa-star.active{color:#ffc64c;filter:drop-shadow(0 0 7px #ff9d30)}.nexa-skill-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}.nexa-skill{display:grid;grid-template-columns:45px 1fr;gap:9px;align-items:center;border:1px solid rgba(104,132,213,.2);border-radius:14px;padding:9px;background:#071129}.nexa-skill-icon{width:45px;height:45px;border-radius:50%;display:grid;place-items:center;font-size:21px;background:radial-gradient(circle,var(--planet),#101a3c 72%);box-shadow:0 0 16px color-mix(in srgb,var(--planet) 34%,transparent);overflow:hidden}.nexa-skill-icon img{width:100%;height:100%;object-fit:cover}.nexa-skill strong{display:block;color:#eef4ff;font-size:10px}.nexa-skill small{display:block;color:#7180a9;font-size:8px;margin:3px 0}.nexa-progress{display:flex;gap:5px}.nexa-segment{flex:1;height:10px;border:1px solid color-mix(in srgb,var(--planet) 45%,transparent);border-radius:99px;background:#09142d}.nexa-segment.active{background:var(--planet);box-shadow:0 0 8px var(--planet)}.nexa-lib-save{border:1px solid rgba(116,224,255,.4)!important;border-radius:14px!important;padding:13px!important;background:linear-gradient(100deg,#7548ff,#168fe1)!important;box-shadow:0 0 22px rgba(55,112,255,.25);font-size:13px}.nexa-role-row{display:flex;gap:7px;margin-bottom:8px}.nexa-role{border:1px solid rgba(126,145,215,.24);border-radius:999px;padding:6px 10px;color:#8391b8;font-size:9px}.nexa-role.active{color:#fff;border-color:var(--planet);background:color-mix(in srgb,var(--planet) 22%,#071129)}
      @media(max-width:520px){.nexa-lib-grid{grid-template-columns:repeat(3,minmax(0,1fr));gap:21px 6px}.nexa-profile-content{padding:14px 10px!important}.nexa-lib-avatar-wrap{width:78px!important;height:78px!important}.nexa-lib-head h4{font-size:11px}.nexa-lib-card.open{max-height:76dvh;padding:16px!important}.nexa-skill-grid{grid-template-columns:1fr 1fr}.nexa-profile-stats{gap:6px!important}.nexa-stat{padding:9px 38px 9px 9px!important}.nexa-stat:after{right:9px!important;font-size:20px!important}}
  ...
  const skillIcon=(skill,i)=>skill?.image_url||skill?.icon_url?`<img src="${esc(skill.image_url||skill.icon_url)}" alt="">`:['⚔','⬡','◎','✦'][i%4];
  function fields(item,row){
    ...
    if(item.item_type==='hero'){
      ...
      const level=Number(p.level||0),stars=Number(p.stars||0),widget=Number(p.widget_level||0);
      return `${control('HERO LEVEL',level||'—',`<div class="nexa-choice-row"><button type="button" class="nexa-choice ${level===80?'active':''}" data-set-max="level" data-max="80">MAXED</button><select data-f="level" class="nexa-choice" aria-label="Hero level">${Array.from({length:80},(_,i)=>`<option value="${i+1}" ${level===i+1?'selected':''}>${i+1}</option>`).join('')}</select></div>`)}${control('STARS',stars||0,`<div class="nexa-stars" data-stars>${[1,2,3,4,5].map(n=>`<button type="button" class="nexa-star ${stars>=n?'active':''}" data-star="${n}">★</button>`).join('')}<input type="hidden" data-f="stars" value="${stars}"></div>`)}<section class="nexa-control"><div class="nexa-control-title"><span>EXPEDITION SKILLS</span><b>MAX LEVEL 5</b></div><div class="nexa-role-row"><span class="nexa-role">JOINER</span><span class="nexa-role active">RALLY LEADER</span></div><div class="nexa-skill-grid">${skills.slice(0,4).map((s,i)=>`<div class="nexa-skill"><span class="nexa-skill-icon">${skillIcon(s,i)}</span><div><strong>${esc(s.name||`Skill ${i+1}`)}</strong><small>${esc(s.effect||'Expedition skill')}</small>${buttons('skill_'+i,5,(p.skills||[])[i]||0)}</div></div>`).join('')}</div></section>${control('WIDGET',widget||0,buttons('widget_level',10,widget))}`;
    }
    if(item.item_type==='expert'){
      const skills=m.skills||[],level=Number(p.level||0);
      return `${control('AFFINITY / RELATIONSHIP',level,`<select data-f="level" class="nexa-choice">${Array.from({length:11},(_,i)=>i*10).map(n=>`<option value="${n}" ${level===n?'selected':''}>${n}${n===100?' • MAXED':''}</option>`).join('')}</select>`)}<section class="nexa-control"><div class="nexa-control-title"><span>EXPERT SKILLS</span><b>SELECT LEVEL</b></div><div class="nexa-skill-grid">${skills.map((s,i)=>{const max=Number(s.max_level)||20,val=Number((p.skills||[])[i]||0);return `<div class="nexa-skill"><span class="nexa-skill-icon">${skillIcon(s,i)}</span><div><strong>${esc(s.name)}</strong><small>MAX EFFECT: ${esc(s.effect||s.max_effect||'See expert data')}</small><select data-f="skill_${i}" class="nexa-choice">${Array.from({length:max+1},(_,n)=>`<option value="${n}" ${val===n?'selected':''}>${n}${n===max?' • MAXED':''}</option>`).join('')}</select></div></div>`}).join('')}</div></section>`;
    }
    if(item.item_type==='pet'){
      const level=Number(p.level||0),skills=m.skills||[{name:'Pet Skill'}];
      return `${control('PET LEVEL',level||'—',`<select data-f="level" class="nexa-choice">${Array.from({length:101},(_,i)=>`<option value="${i}" ${level===i?'selected':''}>${i}${i===100?' • MAXED':''}</option>`).join('')}</select>`)}<section class="nexa-control"><div class="nexa-control-title"><span>PET SKILLS</span><b>VISUAL LEVEL</b></div><div class="nexa-skill-grid">${skills.map((s,i)=>`<div class="nexa-skill"><span class="nexa-skill-icon">${skillIcon(s,i)}</span><div><strong>${esc(s.name||`Skill ${i+1}`)}</strong>${buttons('skill_'+i,10,(p.skills||[])[i]||p.skill_level||0)}</div></div>`).join('')}</div></section>`;
    }
    if(item.item_type==='chief_gear'){
      const tier=p.tier||p.current_tier||'',star=Number(p.stars||0),stage=Number(p.stage||0);
      return `${control('GEAR COLOR / TIER',tier||'Select',`<div class="nexa-choice-row">${['Green','Blue','Purple','Gold','Red'].map((x,i)=>`<button type="button" class="nexa-choice ${tier===x?'active':''}" data-text-choice="tier" style="--planet:${colors[(i+4)%colors.length]}">${x}</button>`).join('')}<input type="hidden" data-f="tier" value="${esc(tier)}"></div>`)}${control('T LEVEL',p.t_level||0,buttons('t_level',8,p.t_level||0))}${control('STARS',star,`<div class="nexa-stars" data-stars>${[1,2,3,4,5].map(n=>`<button type="button" class="nexa-star ${star>=n?'active':''}" data-star="${n}">★</button>`).join('')}<input type="hidden" data-f="stars" value="${star}"></div>`)}${control('STAR PROGRESS',`${stage}/5`,`<div class="nexa-progress" data-segments>${[1,2,3,4,5].map(n=>`<button type="button" class="nexa-segment ${stage>=n?'active':''}" data-segment="${n}"></button>`).join('')}<input type="hidden" data-f="stage" value="${stage}"></div>`)}`;
    }
    const levels=p.charm_levels||[0,0,0],stages=p.charm_stages||[0,0,0];
    return `<div class="nexa-skill-grid">${[0,1,2].map(i=>`<section class="nexa-control"><div class="nexa-control-title"><span>CHARM ${i+1}</span><b>LV ${levels[i]||0}</b></div><span class="nexa-skill-icon">✦</span><select data-charm-level="${i}" class="nexa-choice">${Array.from({length:17},(_,n)=>`<option value="${n}" ${Number(levels[i])===n?'selected':''}>LEVEL ${n}</option>`).join('')}</select><div class="nexa-progress" data-charm-segments="${i}">${[1,2,3,4,5].map(n=>`<button type="button" class="nexa-segment ${Number(stages[i])>=n?'active':''}" data-charm-segment="${n}"></button>`).join('')}</div></section>`).join('')}</div><input type="hidden" data-f="charm_levels" value="${esc(JSON.stringify(levels))}"><input type="hidden" data-f="charm_stages" value="${esc(JSON.stringify(stages))}">`;
  }
