// NEXA TRANSFER WORKSPACE FINISHER V3.5 — CURRENT ALLIANCE GROUP WORDING / V3.4 COMPLETE
(()=>{
'use strict';
...
async function openGroupEditor(g){
 ...
 $('nexaSaveGroup').onclick=async()=>{const st=$('nexaGroupSaveStatus'),payload={group_name:$('nexaGroupName').value.trim(),status:$('nexaGroupStatus').value,plan:$('nexaGroupPlan').value,destination_alliance_tag:$('nexaGroupDest').value,new_alliance_tag:$('nexaGroupNew').value.trim().toUpperCase(),main_contact_ign:$('nexaGroupIgn').value.trim(),main_contact_player_id:$('nexaGroupPid').value.trim(),main_contact_discord:$('nexaGroupDiscord').value.trim(),current_state:$('nexaGroupState').value.trim()};if(!payload.group_name)return st.textContent='Current Alliance is required.';st.textContent='Saving…';const r=await SB.rpc('transfer_workspace_group_save',{p_workspace_id:workspaceId,p_token:token,p_group_id:g?.id||null,p_payload:payload,p_apply_destination_to_members:false});if(r.error||r.data?.ok!==true){st.textContent=r.data?.error||r.error?.message||'Unable to save group.';return}closeModal();await loadGroups();if(g){activeGroup=groups.find(x=>String(x.id)===String(g.id))||null;renderGroupView()}}
}
