...
-- ============================================================
-- NEXA v32.2 — Waiting List + Transfer Roster Fix
-- Run in Supabase SQL Editor BEFORE uploading v32.2.
-- ============================================================

-- The live database uses transfer_status_check.
-- Drop both possible historical names, then recreate one clean constraint.

alter table public.transfer_applications
  drop constraint if exists transfer_status_check;
...
alter table public.transfer_applications
  add column if not exists waiting_list_source_event_id uuid
...
begin
  ...
  set
    ...
    waiting_list_source_event_id=coalesce(
      waiting_list_source_event_id,
      transfer_event_id
    ),
    updated_at=now()
  where id=p_application_id;
...
as $$
  ...
  where
    (public.is_admin() or public.can_manage_transfers())
    and a.transfer_status='next_transfer'
  order by
    a.manual_priority desc,
