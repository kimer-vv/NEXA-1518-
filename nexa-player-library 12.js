/* NEXA Player Intelligence Galactic Library — v12.0 */
(()=>{
  'use strict';
  ...
  const petData={
    ...
    'Snow Ape':{rarity:'SSR',max:100,skill:'Tumbling Power',icon:'🦍',effect:'Squad Deployment Capacity Up',values:[3000,4000,5000,6000,7500,9000,10500,12000,13500,15000],unit:'',duration:'2 hours',cooldown:'20 hours'},
    'Iron Rhino':{rarity:'SSR',max:100,skill:'Rallying Beasts',icon:'🦏',effect:'Rally Capacity Up',values:[30000,40000,50000,60000,75000,90000,105000,120000,135000,150000],unit:'',duration:'2 hours',cooldown:'20 hours'},
  ...
  function fields(item,row){
    ...
    return `<div class="nexa-skill-grid">${[0,1,2].map(i=>{const lv=Number(levels[i]||0),max=charmSteps(lv),stage=Math.min(max,Number(stages[i]||0));return `<section class="nexa-control"><div class="nexa-control-title"><span>CHARM ${i+1}</span><b data-charm-title="${i}">LV ${lv}${max?` · ${stage}/${max}`:''}</b></div><span class="nexa-skill-icon">${['🛡️','🔷','🏹'][i]}</span><div class="nexa-buff-readout" data-charm-readout="${i}">LETHALITY +${charmStat(lv,stage)}% • HEALTH +${charmStat(lv,stage)}%</div><select data-charm-level="${i}" class="nexa-choice">${Array.from({length:19},(_,n)=>`<option value="${n}" ${lv===n?'selected':''}>LEVEL ${n}</option>`).join('')}</select><div class="nexa-progress" data-charm-segments="${i}">${Array.from({length:max},(_,n)=>n+1).map(n=>`<button type="button" class="nexa-segment ${stage>=n?'active':''}" data-charm-segment="${n}"></button>`).join('')}</div></section>`}).join('')}</div><input type="hidden" data-f="charm_levels" value="${esc(JSON.stringify(levels))}"><input type="hidden" data-f="charm_stages" value="${esc(JSON.stringify(stages))}">`;
  }
