// NEXA DISCORD BOT V1.5.0 — GROUP-AWARE APPLICANT / INVITE DISPLAY
...
export function groupLine(a){
  const name=String(a?._group_name||a?.group_name||'').trim();
  if(!a?.group_id&&!name)return'';
  const leader=String(a?._group_leader||a?.group_leader||'').trim();
  return `👥 **${name||'Group'}**${leader?` · Leader: **${leader}**`:''}`;
}
...
export function miniApplicant(a){
  const group=groupLine(a);
  return `**${a.in_game_name||'Applicant'}** · \`${a.player_id||'—'}\`\nFrom: State ${a.current_state||'—'}${a.current_alliance?` · ${a.current_alliance}`:''}\nFurnace: **${a.furnace_level||'—'}** · Power: **${fmtPower(a.current_power)}**\nT12: **${hasT12(a)?'Yes':'No'}** · Account Progress: **${progressionLabel(a.account_progression)}**${group?`\n${group}`:''}`;
}